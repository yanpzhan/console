...
import { Graph, Node, Model, EdgeModel, NodeModel, EventListener, ModelKind, GraphElement, TopologyQuadrant, NodeShape, WithSelectionProps, WithDragNodeProps, WithDndDropProps, WithContextMenuProps, BadgeLocation, NodeStatus, withContextMenu, DragEvent, DragObjectWithType, DragOperationWithType, ConnectorChoice } from '@patternfly/react-topology';
...
import { K8sResourceCommon, K8sResourceKind, K8sResourceKindReference, NamespaceMetrics, PodRCData, WatchK8sResults } from './console-types';
...
export type TopologyResourcesObject = {
    [key: string]: K8sResourceCommon[];
};
export type TopologyDataResources = WatchK8sResults<TopologyResourcesObject>;
export type TopologyDataModelGetter = (namespace: string, resources: TopologyDataResources, workloads: K8sResourceCommon[]) => Promise<Model>;
export declare enum TopologyViewType {
    graph = "graph",
    list = "list"
}
export type ViewComponentFactory = (kind: ModelKind, type: string, view?: TopologyViewType) => React.ComponentType<{
...
export type CreateConnection = (source: Node, target: Node | Graph) => Promise<React.ReactElement[] | null>;
export type CreateConnectionGetter = (createHints: string[], source?: Node, target?: Node) => CreateConnection;
...
export type TopologyDecoratorGetter = (element: Node, radius: number, centerX: number, centerY: number) => React.ReactElement;
...
export type WithEditReviewAccessComponentProps = {
    element: Node;
};
export type WithEditReviewAccess = (verb: K8sVerb) => (WrappedComponent: React.ComponentType) => React.ComponentType<WithEditReviewAccessComponentProps>;
...
export type GetTopologyEdgeItems = (resource: K8sResourceKind, resources: K8sResourceKind[]) => EdgeModel[];
export type GetTopologyGroupItems = (resource: K8sResourceKind) => NodeModel | null;
export type GetTopologyNodeItem = (resource: K8sResourceKind, type: string, data: any, nodeProps?: Omit<OdcNodeModel, 'type' | 'data' | 'children' | 'id' | 'label'>, children?: string[], resourceKind?: K8sResourceKindReference, shape?: NodeShape) => OdcNodeModel;
export type MergeGroup = (newGroup: NodeModel, existingGroups: NodeModel[]) => void;
export type GetModifyApplicationAction = (kind: K8sKind, obj: K8sResourceKind, insertBefore?: string | string[]) => Action;
export type BaseDataModelGetter = (model: Model, resources: TopologyDataResources, workloadResources: K8sResourceKind[], dataModelDepicters?: TopologyDataModelDepicted[], trafficData?: TrafficData, monitoringAlerts?: Alerts) => Model;
export interface KindsMap {
    [key: string]: string;
}
export type GetWorkloadResources = (resources: TopologyDataResources, kindsMap: KindsMap, workloadTypes?: string[]) => K8sResourceKind[];
...
} & WithSelectionProps & WithDragNodeProps & WithDndDropProps & WithContextMenuProps & WithCreateConnectorProps;
...
} & Partial<WithSelectionProps> & Partial<WithDragNodeProps> & Partial<WithDndDropProps> & Partial<WithContextMenuProps> & Partial<WithCreateConnectorProps>;
export type WithContextMenu = <E extends GraphElement>(actions: (element: E) => ActionContext) => ReturnType<typeof withContextMenu>;
...
export type WithCreateConnector = <P extends WithCreateConnectorProps & ElementProps>(onCreate: CreateConnectorWidgetProps['onCreate'], ConnectorComponent?: CreateConnectorRenderer, contextMenuClass?: string, options?: CreateConnectorOptions) => (WrappedComponent: React.ComponentType<Partial<P>>) => React.FC<Omit<P, keyof WithCreateConnectorProps>>;
